...
// 5. Rect* rc = factory.create();	// factory
//-----------------------------------------------

// 변하는 것을 분리하는 패턴 총정리
// #1. 변하는 것을 가상함수로
// => template method : 가상함수가 하는 일이 정책(알고리즘)을 결정
// => factory  method : 가상함수가 하는 일이 "객체의 종류" 결정


// #2. 변하는 것을 다른 클래스로
// => 아래 3개 패턴의 class Diagram 은 완전히 동일
// => 그런데, 뭘교체하는가? 의도는 뭔가 ?
// strategy : 객체가 사용하는 알고리즘 교체(바둑게임의 다음수 예측등)
// state    : 객체의 모든 동작을 한번에 교체
// builder  : 복잡한 객체를 만들때 "동일한 공정(만드는 방법)" 
//			  하지만 각 단계의 표현(결과)는 다른 형태

// 생성패턴 5개 ( 객체를 만들때 사용하는 디자인 패턴 )
// => singleton, prototype, builder, abstract factory, factory method


// 구조 패턴 7개
// 간접층 도입 : adapter, bridge, facade, proxy 
// 재귀적 포함 : composite, decorator
// 속성이 동일하면 공유 : flyweight 


// 행위 패턴 11개
// 변하는 것을 분리 : template method, strategy, state
// 통보, 전달 : observer, chain of responsibility
// 열거, 방문 : iterator, visitor
// undo/redo, 기록 : command(동작취소), memento(상태저장, 메모리증가 될수 있음)

// 관계가 복잡하면 중재자를 만들라 : mediator
// 
// 컴파일러 만들때 사용했던 패턴 : iterpreter 패턴(지금은 거의 사용안함)
